...
# ── GAMMA: catalyst news search (no API key required) ────────────────────────

def _ddg_search(query: str) -> str | None:
    """Single DuckDuckGo news search. Returns top headline or None."""
    try:
        ...
        pass  # neither package installed — skip silently
    except Exception as e:
        print(f"  [WARN] DDG search failed: {e}")
    return None


def fetch_catalyst_news(rows: list[dict]) -> dict[str, str]:
    """
    GAMMA agent: search for catalyst headlines for the most significant movers.
    Targets outliers (abs move > 5%) plus top 3 by absolute change.
    Returns {display_ticker: headline}.
    """
    # Equity-only rows (no indices/gold/btc for news search)
    equity = [r for r in rows if r["ticker"] not in INDEX_TICKERS]

    # Prioritise: outliers first, then top movers, cap at 6 searches
    outliers = [r for r in equity if abs(r["change_pct"]) >= 5.0]
    top3     = sorted(equity, key=lambda r: abs(r["change_pct"]), reverse=True)[:3]
    targets  = list({r["ticker"]: r for r in outliers + top3}.values())[:6]

    catalysts: dict[str, str] = {}
    for r in targets:
        ticker = r["ticker"]
        name   = r["name"]
        print(f"  [GAMMA] Searching catalyst for {ticker}...")
        # Use company name for better results than ticker alone
        headline = _ddg_search(f"{name} stock news today")
        if not headline:
            headline = _ddg_search(f"{ticker} stock")
        if headline:
            catalysts[ticker] = headline
        time.sleep(0.8)  # polite rate-limiting

    return catalysts


def build_catalyst_section(catalysts: dict[str, str], rows: list[dict]) -> str:
    """
    Build Section 7 markdown: catalyst headlines per mover.
    If no catalysts found, returns an empty string (section is omitted).
    """
    if not catalysts:
        return ""
    ...
    ordered = sorted(catalysts.keys(),
                     key=lambda t: abs(row_map[t]["change_pct"]) if t in row_map else 0,
                     reverse=True)
    lines = ["## 7) Catalyst Signals", ""]
    for ticker in ordered:
        row = row_map.get(ticker)
        pct_str = format_pct(row["change_pct"]) if row else ""
        lines.append(f"- **{ticker} ({pct_str}):** {catalysts[ticker]}")
    return "\n".join(lines) + "\n"


# ── Full report assembly ──────────────────────────────────────────────────────

def build_report(slot: str, date_str: str, rows: list[dict],
                 catalysts: dict[str, str] | None = None) -> str:
    ...
    title          = SLOT_TITLES[slot]
    catalyst_sec   = build_catalyst_section(catalysts or {}, rows)
...
def main():
    ...
    print("\n[GAMMA] Fetching catalyst news for top movers...")
    catalysts = fetch_catalyst_news(rows)
    print(f"[GAMMA] Found {len(catalysts)} catalyst headline(s).\n")

    report = build_report(slot, date_str, rows, catalysts)
